...
prepare_table_pef <- function(dt_edge, dt_geneexp, cict_raw_edge_col = 'Spearman')
{
    ...
    dt_edge <- dt_edge %>% dplyr::mutate(Weight=.data[[cict_raw_edge_col]]) %>% select(src,trgt,everything()) #make src and trg the first 2 columns
...
calculate_f0 <- function (results)
{
  ...
  dt_vertices <- dt_vertices %>%
    ...
    mutate(Indegree=ifelse(is.na(Indegree), pseudo_zero, Indegree),
  ...
  dt_vertices <- dt_vertices %>%
    mutate(probInp=OcrInp/vertexOcrSumIn, probOut=OcrOut/vertexOcrSumOut)

  vinfcols <- c("gene", "OcrInp", "OcrOut", "probInp", "probOut", "Indegree", "Outdegree")
  ...
  dt_vertices_trgtsum <- dt_vertices %>%
    ...
    group_by(gene) %>%
    summarise(SumOcrInp.y=sum(OcrInp.y, na.rm=TRUE), SumOcrOut.y=sum(OcrOut.y, na.rm=TRUE))

  dt_vertices_srcsum <- dt_vertices %>%
    left_join(dt_edge, by=c("gene"="trgt")) %>%
    group_by(gene) %>%
    summarise(SumOcrInp.x=sum(OcrInp.x, na.rm=TRUE), SumOcrOut.x=sum(OcrOut.x, na.rm=TRUE))
  ...
  cols <- c("SumOcrInp.x", "SumOcrOut.x", "SumOcrInp.y", "SumOcrOut.y", "OcrOut", "OcrInp")

  # Convert factors to numeric
  factorToNumeric <- function(x) as.numeric(as.character(x))
  dt_vertices_num <- setDT(dt_vertices)[, lapply(.SD, factorToNumeric), .SDcols=cols]
  dt_vertices <- dt_vertices[, !names(dt_vertices) %in% cols, with=FALSE]
  dt_vertices <- cbind(dt_vertices_num, dt_vertices)
  rm(dt_vertices_num); gc()
  ...
  vinfcols <- c("gene", "SumOcrInp.x", "SumOcrOut.x", "SumOcrInp.y", "SumOcrOut.y")
  dt_edge <- dt_edge %>%
    inner_join(dt_vertices[, ..vinfcols], by=c("src"="gene")) %>%
    inner_join(dt_vertices[, ..vinfcols], by=c("trgt"="gene"))

  # Replace NA values in dt_edge
  dt_edge <- replace_na(dt_edge, replace=list(SumOcrOut.x.x=0, SumOcrOut.x.y=0,
  ...
  dt_edge <- dt_edge %>%
    ...
    mutate(conf=ifelse(is.na(conf) | is.infinite(conf), 0, conf),
...
calculate_moments <- function(data, group_col, value_col, prefix) {
    lmoments <- data %>% dplyr::group_by(!!sym(group_col)) %>% do(extractLmoments(.[[value_col]]))
    summary_stats <- data %>% group_by(!!sym(group_col)) %>% summarise(
      ...
      MAD = myMedianAbsoluteDeviation(!!sym(value_col), Median, MADconst, na.rm = TRUE, default = 0),
      ...
      NMAD = myMedianAbsoluteDeviation(!!sym(paste0(value_col, "N")), NMedian, NMADconst, na.rm = TRUE, default = 0)
    )
    full_stats <- summary_stats %>% inner_join(lmoments, by = group_col)
...
calculate_f1 <- function(results2)
{
  ...
  dt.geneexp <- results2[[2]] # unmodified dt_geneexp, expression data
  ...
  dt.vertices = dt.vertices %>% left_join(powerParamsIn,by=c("gene"="trgt"))
  dt.vertices = dt.vertices %>% left_join(powerParamsOut,by=c("gene"="src"))
  ...
  vinfcols=c("gene","Pinsum","PinAbsSum","PinSD","PinMean","Poutsum","PoutAbsSum","PoutSD","PoutMean")
  ...
  dt.edge = dt.edge %>% inner_join(dt.vertices[,vinfcols],by=c("src"="gene"))
  dt.edge = dt.edge %>% inner_join(dt.vertices[,vinfcols],by=c("trgt"="gene"))
  ...
  if(TRUE){
    ...
    dt.edge1 = dt.edge
    dt.edge1=plyr::rename(dt.edge, replace=c( 'Weight' ='IBA' ,'UR' ='URBA' ,'AM' ='AMBA' ,'OEER' ='OEERBA' ,'OERR' ='OERRBA' ,
    ...
    dt.edge2 = dt.edge %>% left_join(dt.edge1, by=c("src"="trgt","trgt"="src"))
    rm(dt.edge1)
    a=sapply(dt.edge2, function(x) sum(is.na(x)));a[a>0]
    b=sapply(dt.edge2, function(x) sum(x==0 & !is.na(x)));b[b>0]
    ...
    dt.edge2 = replace_na(dt.edge2,replace=
    ...
    a=sapply(dt.edge2, function(x) sum(is.na(x)));a[a>0]
    rm(dt.edge);gc()
    dt.edge= dt.edge2 %>%dplyr::mutate(confDiff=conf-confBA,contribDiff=contrib-contribBA,
  ...
  dt.vertices.back=dt.vertices
  dt.edge.tmp=
    dt.vertices.outflux = dt.vertices %>% inner_join(dt.edge[,c("src","Weight")],by=c("gene"="src")) %>%
  ...
  dt.vertices.influx = dt.vertices %>% inner_join(dt.edge[,c("trgt","Weight")],by=c("gene"="trgt")) %>%
    group_by(gene) %>% summarise(influx=sum(Weight))

  dt.vertices= dt.vertices %>% left_join(dt.vertices.outflux,by=c("gene"="gene"))
  dt.vertices= dt.vertices %>% left_join(dt.vertices.influx,by=c("gene"="gene"))
  ...
  rm(dt.vertices.influx,dt.vertices.outflux,powerParamsIn,powerParamsOut,dt.edge.tmp)
  setDT(dt.edge);dt.edge.noselfedge=dt.edge [src!=trgt,.(src,contrib,conf,trgt)]
  setDF(dt.edge);setDF(dt.vertices)
...
dt.vertices.back1 <- dt.vertices
  ...
  othersConfsFull <- calculate_moments(dt.edge, "trgt", "conf", "ocf")
  othersContribsFull <- calculate_moments(dt.edge, "trgt", "contrib", "ocb")
  # Calculate moments for self contributions and confidence
  selfConfsFull <- calculate_moments(dt.edge, "src", "conf", "scf")
  selfContribsFull <- calculate_moments(dt.edge, "src", "contrib", "scb")


  dt.vertices.othersparams <- othersConfsFull %>% inner_join(othersContribsFull, by = c("ocftrgt" = "ocbtrgt"))
  dt.vertices.selfparams <- selfConfsFull %>% inner_join(selfContribsFull, by = c("scfsrc" = "scbsrc"))


  a <- sapply(dt.vertices.othersparams, function(x) sum(is.na(x)))
  a[a > 0]
  ...
  dt.vertices = dt.vertices %>% left_join(dt.vertices.othersparams,by=c("gene"="ocftrgt"))
  dt.vertices = dt.vertices %>% left_join(dt.vertices.selfparams,by=c("gene"="scfsrc"))
...
dt.vertices = my_replace_na(dt.vertices, rplist = list(Mean=0, Median=0, Skew=0, Kurt=3,
                                                       ...
                                                       MADcost=1.488, L1=0, L2=0, tau3=0, tau4=0.1226))

rm(dt.vertices.othersparams, dt.vertices.selfparams, othersConfsFull, othersContribsFull)
...
calculate_f2 <- function (results3)
{
    ...
    dt.vertices <- results3[[2]] # unmodified dt_geneexp, expression data
  ...
  colList=c('gene','DiagnosesAb','evenTyp','Pinsum','PinBAsum','Poutsum','PoutBAsum',
